...
prompt_customwifi() {
  ...
  if [[ $(nmcli device wifi | grep \*) = '' ]] ; then
    ui="\uf6ff"
    color='blue'
  else
    local signal=$(nmcli device wifi | grep "*" | awk '{print $7}')
    [[ $signal -gt 75 ]] && color='green'
    [[ $signal -lt 40 ]] && color='red'
    ui="\ufaa8$signal%f"
  fi
...
POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(virtualenv command_execution_time docker_machine status root_indicator background_jobs time battery customwifi)
